...
const CustomerTable = () => {
  ...
  rows.forEach((row) => {
    ...
    if (!uniqueEmails[email]) {
      ...
      // Check if the email is a duplicate, but the has_account is true
      const existingRow = uniqueRows.find((uniqueRow) => uniqueRow.original.email === email);
      if (existingRow) {
        existingRow.has_account = true;
      }
    }
  });
            ...
            return (
              ...
              >
                {row.cells.map((cell, index) => {
                  if (cell.render("Header") === "User") {
                    return (
                      <Table.Cell {...cell.getCellProps()}>
                    {toTitleCase(row.original.has_account.toString())} 
                    </Table.Cell>
                    )
                  } else {
                    return (
