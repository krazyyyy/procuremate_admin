  ...
  const onSubmit = (data: MarkShippedFormData) => {
    ...
    action.mutate(requestObj, {
      onSuccess: async () => {
        ...
        const allItemsShipped = updatedItems.every(
          (item) => item.metadata.status === "Shipped"
        );
        ...
        if (allItemsShipped) {
          ...
          updatedItems.some((item) => item.metadata.status === "Shipped")
        ) {
          orderStatus = "Partially Shipped";
        } else {
          orderStatus = order?.metadata?.status || "";
        }
    
        try {
          await Medusa.orders.updateItem(orderId, {
            metadata: {
              status: orderStatus,
            }
          });
          await Medusa.sendProduction.statusEmail({
            email : order?.email,
            description: `Currenctly Your Order is being ${orderStatus}`,
            title: orderStatus
          })
