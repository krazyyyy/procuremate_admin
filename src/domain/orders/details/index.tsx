...
const OrderDetails = () => {
  ...
  const handleDateChange = async (setSelectedDate) => {
  
    try {
      if (openDatePicker === 'due_date') {
        await Medusa.orders.updateItem(id, {
          metadata: {
            due_date: setSelectedDate
          }
        });
        updateOrder(
        notification("success", "Updated Due Date", "success")
        )
        // Handle successful update for the due_date field
      } else if (openDatePicker === 'send_date') {
        await Medusa.orders.updateItem(id, {
          metadata: {
            send_date: setSelectedDate
          }
        });
        updateOrder(
          notification("success", "Updated Send Date", "success")
        )
  ...
  const getData = async () => {
       const data = await Medusa.jobCards.list(10, 1, `job_order_id=${id}`)
      // getJobData(data.data.jobCards.job_cards.custom_design_id?.design_data?.design)
      const jobCards = data.data.jobCards.job_cards;
      // console.log(jobCards)
      let total = 0;
      jobCards.forEach(async (jobCard) => {
        // const data = await Medusa.jobCards.list()
        // const designData = jobCard.custom_design_id?.design_data?.design;
        const job_data = await Medusa.customDesign.retrieve(jobCard.custom_design_id?.id)
        const designData = (job_data?.data?.custom_design[0]?.design_data?.design)
        console.log(designData)
        if (designData && Array.isArray(designData)) {
          designData.forEach((design) => {
            
            if (design.price) {
              const priceValue = parseFloat(design.price) * 100;
      ...
      getJobData(data.data.jobCards.job_cards)
  }
  ...
  // !capturePayment.isLoading && payment_status === "awaiting"
        ...
        {isLoading || !order ? (
            ...
            <div className="flex space-x-4">
              <div className="flex h-full w-7/12 flex-col">
                <BodyCard
                  ...
                  status={<OrderStatusComponent status={order?.metadata?.status || order.status} />}
                  forceDropdown={true}
                  actionables={[
                    {
                      label: "Cancel Order",
          ...
          <div className="flex items-center">
            ...
            >
              ...
              {moment(order?.metadata?.due_date || "N/A").format(
                            "D MMMM YYYY"
                          ) || "N/A"}
            </button>
          </div>
          ...
          <div className="flex items-center">
            <button 
              ...
              >
              <ClockIcon className="h-5 w-5 mr-1" />
              <div>{moment(order?.metadata?.send_date || "N/A").format(
                            "D MMMM YYYY"
                          ) || "N/A"}</div>
            </button>
          </div>
          ...
          {openDatePicker === 'send_date' && (
                    ...
                    >
                      <div className="mt-6">
                        ...
                        <DisplayTotal
                          ...
                          totalTitle={`Total Material Cost`}
                        />
                        <DisplayTotal
                          variant={"large"}
                          currency={order.currency_code}
                    ...
                    <div className="flex w-full items-center space-x-4">
                      ...
                      <div>
                        ...
                        {order.shipping_address && (
                          <span className="inter-small-regular text-grey-50">
                            {order.shipping_address.city} {order?.shipping_address?.metadata?.country},{" "}
                            { order?.shipping_address?.metadata?.country||
                              isoAlpha2Countries[
            ...
            {showEmailTemplate && (
              <CreateEmail
                email={order.email}
                handleCancel={() => setShowEmailTemplate(false)}
                orderId={order.id}
              />
            )}
            {showRefund && (
