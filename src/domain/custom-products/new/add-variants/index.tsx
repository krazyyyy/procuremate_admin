...
const AddVariantsForm = ({
  ...
  const onUpdateVariant = (index: number, data: CreateFlowVariantFormType) => {
    ...
    } // We can be sure that the value is set at this point.
  ...
  const onToggleForm = () => {
    reset(createDefaultVariant(options[0]))
    toggle()
  }
...
const createDefaultVariant = (
  ...
  return {
    _internal_id: uuidv4(),
    general: {
      title: "Black", // Set the default title here
      material: null,
    },
    prices: {
      prices: [],
    ...
    options:
      option
        ? [{ title: option.title, option_id: option.id, option: null }]
        : [],
  }
}
